...
export function buildSignerExternalUrl(token: string): string {
  const base = process.env.APP_URL ?? "http://localhost:3000";
  return `${base}/external/signer/${token}`;
}
...
export async function resolveSignerMagicLinkUrl(workflowId: string): Promise<string | null> {
  const supabase = createServiceClient();
  const { data: workflow } = await supabase
    .from("workflow_instances")
  ...
  await saveWorkflowMagicLinkUrl(workflowId, signerUrl);
  return signerUrl;
}
