...
import { sendSignerRequestEmail } from "./sendRequest";
import { createAuditLog } from "@/lib/audit/createAuditLog";

export async function resendSignerRequest(workflowId: string, options?: { ccMe?: boolean }) {
  ...
  const result = await sendSignerRequestEmail(workflowId, options);
...
export async function revokeSignerMagicLink(workflowId: string) {
  ...
  await createAuditLog({
    ...
    action: "signer_magic_link.revoked",
