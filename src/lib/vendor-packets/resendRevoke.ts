...
import { sendVendorPacketEmail } from "./sendPacket";
import { createAuditLog } from "@/lib/audit/createAuditLog";

export async function resendVendorPacketRequest(workflowId: string, options?: { ccMe?: boolean }) {
  ...
  if (!workflow) throw new Error("Vendor packet not found");
  ...
  const result = await sendVendorPacketEmail(workflowId, options);
...
export async function revokeVendorPacketMagicLink(workflowId: string) {
