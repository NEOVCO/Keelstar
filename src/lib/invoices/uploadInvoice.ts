...
import {
  INVOICE_ALLOWED_MIME,
  INVOICE_MAX_FILE_SIZE,
...
export function buildInvoiceStoragePath(
  ...
  return `organizations/${organizationId}/invoices/${workflowId}/documents/${documentId}/versions/v${versionNumber}/${safe}`;
}
...
export async function uploadInvoiceDocument(
  ...
  if (!INVOICE_ALLOWED_MIME.includes(file.mimeType as (typeof INVOICE_ALLOWED_MIME)[number])) {
    throw new Error("File type not allowed. Accepted: PDF, PNG, JPEG");
  }
  if (file.sizeBytes > INVOICE_MAX_FILE_SIZE) {
  ...
  if (TERMINAL(workflow.status)) {
    throw new Error("Cannot upload for a closed invoice");
  }
  ...
  await createAuditLog({
    ...
    action: "invoice.document_uploaded",
    targetType: "document",
    targetId: document.id,
    metadata: { workflowId, versionNumber, filename: safeFilename },
  });

  trackEvent("invoice_uploaded", { workflowId });
