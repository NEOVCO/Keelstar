...
export function buildPolicyExternalUrl(token: string): string {
  const base = process.env.APP_URL ?? "http://localhost:3000";
  return `${base}/external/policy/${token}`;
}
...
export async function resolvePolicyMagicLinkUrl(workflowId: string): Promise<string | null> {
  const supabase = createServiceClient();
  const { data: workflow } = await supabase
    .from("workflow_instances")
  ...
  await saveWorkflowMagicLinkUrl(workflowId, policyUrl);
  return policyUrl;
}
