...
export function CoiWorkflowReviewActions({
  workflowId,
  status,
  canApprove,
}: {
  ...
  async function action(name: string, body: Record<string, unknown> = {}) {
    setLoading(name);
    const res = await fetch(`/api/coi/requests/${workflowId}`, {
      method: "POST",
    ...
    if (name === "export" && res.ok) {
      ...
      a.href = url;
      a.download = `coi-evidence-${workflowId}.csv`;
      a.click();
      setLoading(null);
      return;
    }
  ...
  return (
    <div className="flex flex-wrap gap-2">
      ...
      {["active_monitoring", "expiring_soon", "expired", "approved"].includes(status) && canApprove && (
