...
import { W9UploadForm } from "@/components/forms/W9UploadForm";
import { formatDate } from "@/lib/utils/cn";

export default async function ExternalW9Page({ params }: { params: { token: string } }) {
  ...
  if (workflow && ["completed", "approved", "cancelled"].includes(workflow.status)) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-bg px-4">
  ...
  trackEvent("magic_link_opened", { workflowId: result.link.workflowInstanceId });
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-bg px-4">
      <div className="w-full max-w-lg">
        ...
        <Card>
          <CardHeader>
            <CardTitle>W-9 upload request</CardTitle>
            <CardDescription>
              {org?.name} is requesting your completed W-9 form.
            </CardDescription>
          </CardHeader>
